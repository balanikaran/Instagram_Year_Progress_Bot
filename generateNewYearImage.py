...
def generateAndSave(progressPercent):
    ...
    fntHappyNewYear = ImageFont.truetype('fonts/Product Sans Regular.ttf', 40)
    ...
    draw.text(((Width-w)/2, (Height-h)/2 + 20), percent, fill=white, font=fntPercent)

    progressString = generateStyleString(progressPercent)
    w, h = fntGraph.getsize(progressString)
    draw.text(((Width-w)/2, (Height-h)/2 + 100), progressString, fill=white, font=fntGraph)

    newYearWish = "* HAPPY NEW YEAR! *"
    w, h = fntHappyNewYear.getsize(newYearWish)
    draw.text(((Width-w)/2, h + 80), newYearWish, fill=white, font=fntHappyNewYear)
